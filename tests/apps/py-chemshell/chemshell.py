...
The package py-chemshell allows creating a Python script
which make use of external chemical simulation codes.
The packages runs locally and creates task farms and other workflows.
Tasks are submitted to slurm and executed on the compute nodes.
...
def flatten_options(options: dict) -> str:
    ...
    chemshell_opt_flattened = " ".join([f"--{key}={value}" for key, value in options.items()])
...
class PyChemshellBase(rfm.RunOnlyRegressionTest):
    ...
    account = (
        "z19"  # Hard coded. Could the default be obtained from a configuration file ? Can be overriden from the cli
    )
    ...
    @run_before("run")
    def set_chemshell_args(self):
        """Sets the argument and options to the chemshell script"""
...
class PyChemshellH20HF(PyChemshellBase):
    ...
    Computes Hartree-Fock energy for a water molecule
    using NWchem as a backend.
    It is mainly a functionality test, as the test
    is too small to be significant for measuring performance.
